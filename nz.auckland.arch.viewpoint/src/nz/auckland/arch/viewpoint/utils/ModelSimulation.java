...
//	Hashtable<String, DRepresentationElement>  dgHash = new Hashtable<String, DRepresentationElement>();
		...
		for (DView view : root.getOwnedViews()) {
			for (DRepresentationDescriptor dr : view.getOwnedRepresentationDescriptors()) {
				...
				if (dr.getDescription().getName().equals("PortDiagram")) {
					...
					for (DRepresentationElement element : rpst.getRepresentationElements()) {
						System.out.println("		semantic elements:" + element.getSemanticElements().size());
						try {
							if (element.getSemanticElements() != null && element.getSemanticElements().size() > 0) {
								EObject obj = element.getSemanticElements().get(0);
								System.out.println("				obj: " + obj.toString());

								if (obj != null && "Port".equals(obj.eClass().getName())) {
		...
		while (st.hasMoreTokens()) {
			...
			if (eventLabel.indexOf('!') != -1 || eventLabel.indexOf('?') != -1) {
				...
				if (this.portRpstHash.containsKey(nextCallElement)) {
					 ...
					 * else if (this.connHash.containsKey(nextCallElement)) { String connName =
					 * nextCallElement; System.out.println("\tflashing "+connName);
					 * //flashElement(session.getSession(),this.systemDiagramRpst, domain,
					 * connHash.get(connName)); String role = eventSt.nextToken();
					 * 
					 * if(roleHash.containsKey(role)) { System.out.println("\tflashing "+role);
					 * flashElement(session.getSession(),this.systemDiagramRpst, domain,
					 * roleHash.get(role)); } }
					 */
	...
	public void flashElement(Session session, DRepresentation representation, TransactionalEditingDomain domain,
